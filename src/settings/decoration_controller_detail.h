...
// Shared inline helpers between decorationpagecontroller.cpp and
// decorationpagecontroller_paths.cpp. The two TUs split the same class
// across files to stay under the 800-line cap; both convert surface-pack
// effect / parameter values to QVariantMap for QML, and both build the
// sparse / resolved DecorationProfile -> QVariantMap projections. Inline
// definitions here ensure each TU gets its own copy without relying on
// unity-build TU merging for cross-TU linkage.
